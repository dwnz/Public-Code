...
var TradmeOAuth = WinJS.Class.define(
    //Constructor
    function (consumerKey, consumerSecret, accessToken, accessTokenSecret, callbackUrl) {
        this._consumerKey = consumerKey;
        this._consumerSecret = consumerSecret;

        //If we already have accessToken then doTrademeWebAuth can be avoided
        ...
        this._callbackURL = callbackUrl || 'http://localhost';
    },
    //instanceMembers
    {
        _authorizeURL: 'https://secure.trademe.co.nz/Oauth/Authorize',
        _requestTokenURL: 'https://secure.trademe.co.nz/Oauth/RequestToken',
        _accessTokenURL: 'https://secure.trademe.co.nz/Oauth/AccessToken',
         ...
         **/
        doTrademeWebAuth: function (callback) {
            var self = this,
                i,
                splits,
                responseKeyValPairs,
            ...
            this._getRequestToken(function (requestTokenData) {
                ...
                Windows.Security.Authentication.Web.WebAuthenticationOptions.none, startURI, endURI)
                    .done(function (result) {
                        var trademeResponseData = result.responseData,
                            trademeResponseStatus = result.responseStatus;

                        if (trademeResponseStatus === Windows.Security.Authentication.Web.WebAuthenticationStatus.errorHttp) {
                            //console.log("Error returned: " + result.responseErrorDetail);
                            callback(false);
                        } else {
                            //The authorize request when successful will return an oauthVerifier
                            //https://dev.twitter.com/docs/auth/authorizing-request
                            responseKeyValPairs = trademeResponseData.split("?")[1].split("&");
